    ...
    <div className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold">🌳 myTree</h1>
            <p className="text-gray-600">{trees.length} trees planted</p>
          </div>
          <div className="flex gap-4">
            <button
              onClick={() => setShowAddForm(true)}
      ...
      {showAddForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h2 className="text-xl font-bold mb-4">Add New Tree</h2>
            <SimpleAddForm onClose={() => setShowAddForm(false)} onSuccess={fetchTrees} />
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => setSelectedTree(null)} className="text-gray-500 hover:text-gray-700 text-xl">×</button>
            </div>
            <img src={selectedTree.image_url} alt={selectedTree.name} className="w-full h-48 object-cover rounded mb-4" />
            {selectedTree.species && <p><strong>Species:</strong> {selectedTree.species}</p>}
            <p><strong>Planted:</strong> {new Date(selectedTree.planted_date).toLocaleDateString()}</p>
            <p><strong>Location:</strong> {selectedTree.latitude.toFixed(4)}, {selectedTree.longitude.toFixed(4)}</p>
            {selectedTree.description && <p className="mt-2"><strong>Story:</strong> {selectedTree.description}</p>}
            <button
...
function SimpleAddForm({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    name: '',
    species: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.imageUrl || !formData.latitude || !formData.longitude) {
